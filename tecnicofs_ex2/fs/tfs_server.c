...
int tfs_mount() {
    ...
    if(pthread_mutex_lock(&session_lock) < 0) exit(1);
    ...
    if(pthread_mutex_unlock(&session_lock) < 0) exit(1);

    if((sessions[session_id].client_pipe = open(pipename, O_WRONLY)) < 0) exit(1);
...
int tfs_shutdown_after_all_closed(char *server_path) {
    ...
    if(return_value != -1) {
        ...
        if(pthread_mutex_destroy(&session_lock) < 0) exit(1);
        close(server_pipe);
        unlink(server_path);
...
int main(int argc, char **argv) {
    ...
    /* TO DO */
    if(pthread_mutex_init(&session_lock, NULL)) exit(1);
